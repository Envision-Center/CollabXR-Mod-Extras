...
namespace CollabXR.ModExtras
{
	...
	public class PlaybackDirector : MonoBehaviour, IPlaybackDirector
	{
		...
		public string EditorGuid => _editorGuid;
		...
		private void EnsureEditorGuid()
		{
			...
			{
				_editorGuid = GUID.Generate().ToString();
		...
		{
			foreach (var effect in _effects)
				if (effect is IScrubbableEffect s)
					s.OnScrubStart();
		}
		...
		private void DriveEffects(float percent)
		{
			foreach (var effect in _effects)
			{
				if (effect is MonoBehaviour mb && mb != null)
				{
					...
					float effectPercent = (syncMode == PlaybackSyncMode.ScalePercent || effect is ISubFrameEffect) ? percent : CalculateFrameBasedPercent(percent, effect.FrameCount);
		...
		private void InitializeViewModel()
		{
			...
			viewModel.OnComponentChangeRequested += (
				(id, index) =>
				{
					if (_locallyDriven)
					{
